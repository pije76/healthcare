...
#		self.fields['ec_ec_ic_number'].help_text = _('*IC Number format needs to be yymmdd-xx-zzzz.')
...
#	class Meta:
#		model = Admission
#		fields = 'ec_name', 'ec_ec_ic_number', 'ec_relationship', 'ec_phone', 'ec_address'
#		widgets = {
#			'patient': forms.HiddenInput(),
#			'age': forms.HiddenInput(),
#		}
...
#	AdmissionFormSet = inlineformset_factory(Admission, form=AdmissionForm, fields=['ec_name', 'ec_ec_ic_number', 'ec_relationship', 'ec_phone', 'ec_address'], extra=1, can_delete=True)
...
#	family_name = forms.ModelChoiceField(queryset=Admission.objects.all(), widget=autocomplete.ModelSelect2(url='application-homeleave'))
#	family_ic_number = forms.CharField(max_length=14, required=False, label="", validators=[ic_number_validator], widget=forms.TextInput(attrs={'class': "form-control", 'placeholder': _("yymmdd-xx-zzzz")}))
	family_ic_number = forms.CharField(required=False, label="", widget=AutoCompleteWidget(ECNumberLookup, attrs={'class': "form-control", 'placeholder': _("yymmdd-xx-zzzz")}))
