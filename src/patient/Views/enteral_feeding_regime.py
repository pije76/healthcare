...
def enteral_feeding_regime_list(request, username):
    ...
    enteralfeedingregime_data = EnteralFeedingRegime.objects.filter(
        patient=patientid).exclude(type_of_milk__isnull=True).exclude(amount__exact='0')
    ...
    get_lastdate = EnteralFeedingRegime.objects.filter(patient=patientid).order_by(
        '-date').exclude(amount__isnull=True).values_list('date', flat=True).first()
    get_warm_water_before = EnteralFeedingRegime.objects.filter(patient=patientid).filter(
        date=get_lastdate).values_list('warm_water_before', flat=True).first()
    get_warm_water_after = EnteralFeedingRegime.objects.filter(patient=patientid).filter(
        date=get_lastdate).values_list('warm_water_after', flat=True).first()
    get_warm_water_total = EnteralFeedingRegime.objects.filter(patient=patientid, date=get_lastdate).filter(
        amount__isnull=False).exclude(amount__exact='0').annotate(total_warm=F('warm_water_before') + F('warm_water_after') + F('amount'))
    total_fluids = get_warm_water_total.aggregate(
        Sum('total_warm'))['total_warm__sum'] if get_warm_water_total else 0
    ...
    if request.method == "GET":
        ...
        warm_water_before = EnteralFeedingRegime.objects.filter(
            patient=patientid, date=set_newdate).values_list('warm_water_before', flat=True).first()
        warm_water_after = EnteralFeedingRegime.objects.filter(
            patient=patientid, date=set_newdate).values_list('warm_water_after', flat=True).first()
        total_feeding = EnteralFeedingRegime.objects.filter(
            patient=patientid).filter(date=set_newdate).aggregate(Sum('amount'))
        total_feeding = total_feeding['amount__sum'] if total_feeding else 0
        get_warm_water_total = EnteralFeedingRegime.objects.filter(patient=patientid, date=set_newdate).filter(
            amount__isnull=False).exclude(amount__exact='0').annotate(total_warm=F('warm_water_before') + F('warm_water_after') + F('amount'))
        total_fluids = get_warm_water_total.aggregate(
            Sum('total_warm'))['total_warm__sum'] if get_warm_water_total else 0
    ...
    if request.method == "POST" and request.is_ajax:
        ...
        warm_water_after = EnteralFeedingRegime.objects.filter(
            patient=patientid, date=set_newdate).values_list('warm_water_after', flat=True).first()
        totalfeeding = EnteralFeedingRegime.objects.filter(
            patient=patientid).filter(date=set_newdate)
        total_feeding = totalfeeding.aggregate(
            Sum('amount'))['amount__sum'] if totalfeeding else 0
        get_warm_water_total = EnteralFeedingRegime.objects.filter(patient=patientid, date=set_newdate).filter(
            amount__isnull=False).exclude(amount__exact='0').annotate(total_warm=F('warm_water_before') + F('warm_water_after') + F('amount'))
        total_fluids = get_warm_water_total.aggregate(
            Sum('total_warm'))['total_warm__sum'] if get_warm_water_total else 0
...
#	EnteralFeedingRegime_FormSet = formset_factory(EnteralFeedingRegime_Form, extra=24, formset=TestBaseFormSet)
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            for item in formset:
                get_warm_water_before = EnteralFeedingRegime.objects.filter(
                    patient=patients).values_list("warm_water_before", flat=True).first()
                get_warm_water_after = EnteralFeedingRegime.objects.filter(
                    patient=patients).values_list("warm_water_after", flat=True).first()
