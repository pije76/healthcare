...
#	profiles = UserProfile.objects.filter(pk=patientid)
    full_name_profiles = UserProfile.objects.filter(
        username=username).values_list('full_name', flat=True).first()
    profiles = UserProfile.objects.filter(pk=patientid)
#	allergies = MedicationAdministrationRecord.objects.filter(patient=patientid).values_list('allergy', flat=True).first()
    medicine_date = MedicationAdministrationRecord.objects.filter(
        patient=patientid).values_list('medication_template__medication_date', flat=True).first()
#	medicine_data = MedicationAdministrationRecord.objects.filter(patient=patientid)
    get_lastdate = MedicationAdministrationRecord.objects.filter(patient=patientid).order_by('-medication_template__medication_date').exclude(
        medication_template__medication_time__isnull=True).values_list('medication_template__medication_date', flat=True).first()
    medicine_data = MedicationAdministrationRecord.objects.filter(patient=patientid).filter(
        medication_template__medication_date=get_lastdate).exclude(medication_template__medication_time__isnull=True)
    medicine_stat = MedicationAdministrationRecord.objects.filter(
        patient=patientid).exclude(medication_template__medication_time__isnull=True)
    allergy_drug_data = Allergy.objects.filter(
        patient=patientid).values_list('allergy_drug', flat=True).first()
    allergy_food_data = Allergy.objects.filter(
        patient=patientid).values_list('allergy_food', flat=True).first()
    allergy_others_data = Allergy.objects.filter(
...
def medication_administration_create(request, username):
    ...
    usernameid = UserProfile.objects.filter(
    ...
    patients_templates = MedicationAdministrationRecord.objects.filter(patient=patientid).filter(medication_template__medication_time__in=[
        '00:00', '01:00', '02:00', '03:00', '04:00', '05:00', '06:00', '07:00', '08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00', '20:00', '21:00', '22:00', '23:00']).order_by("medication_template__medication_time")
    mar_profiles = MedicationAdministrationRecord.objects.filter(
        patient=patientid)
