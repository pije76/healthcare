...
#			patient_profile = PatientProfile(patient_name=patientname)
#			patient_profile.full_name = patient_form.cleaned_data['full_name']
#			patient_profile.username = patient_form.cleaned_data['username']
#			patient_profile.email = patient_form.cleaned_data['email']
#			patient_profile.password = patient_form.cleaned_data['password']
#			patient_profile.date_joined = patient_form.cleaned_data['date_joined']
#			patient_profile.ic_number = patient_form.cleaned_data['ic_number']
#			patient_profile.ic_upload = patient_form.cleaned_data['ic_upload']
#			patient_profile.birth_date = patient_form.cleaned_data['birth_date']
#			patient_profile.age = patient_form.cleaned_data['age']
#			patient_profile.gender = patient_form.cleaned_data['gender']
#			patient_profile, created = PatientProfile.objects.update_or_create(
...
#				defaults=None,
#			)
#			PatientProfile.objects.filter(pk=profile.id).update(
#				full_name=profile.full_name,
#				username=profile.username,
#				email=profile.email,
#				password=profile.password,
...
#				gender=profile.gender,
#			)
##			getpatient = UserProfile.objects.get(id=profile.id)
##			patient_profile = getpatient.patientprofile
#			getpatient = UserProfile.objects.get(
#				full_name=profile.full_name,
#				username=profile.username,
#				email=profile.email,
#				password=profile.password,
...
#			)
#			patient_profile = PatientProfile()
#			patient_profile.__dict__.update(getpatient.__dict__)
#			patient_profile.full_name = profile.full_name
#			patient_profile.username = profile.username
#			patient_profile.email = profile.email
#			patient_profile.password = profile.password
#			patient_profile.date_joined = profile.date_joined
#			patient_profile.ic_number = profile.ic_number
#			patient_profile.age = profile.age
#			patient_profile.gender = profile.gender
##			patient_profile.save()
		...
		if ec_form.is_valid():
			for item in ec_form:
				ec_profile = EmergencyContact_FormSet()
			...
			messages.success(request, _(page_title + ' form was created.'))
			return redirect('patient:patientdata_detail', username=patientusername)
			print("formec1: ", ec_form)
		else:
			messages.warning(request, ec_form.errors)
			print("formec2: ", ec_form)
		...
		patient_form = UserProfile_ModelForm(instance=patients, prefix="patient_form")
...
#	}
	...
	form_class = UserProfile_ModelForm
	page_title = _('Admission Form')
