...
def intake_output_list(request, username):
    ...
    total_intake = IntakeOutput.objects.annotate(sum_abc=F('intake_oral_ml') + F(
        'intake_parenteral_ml') + F('intake_other_ml')).aggregate(total=Sum('sum_abc'))['total'] or 0
    total_output = IntakeOutput.objects.annotate(sum_abc=F('output_urine_ml') + F(
        'output_gastric_ml') + F('output_other_ml')).aggregate(total=Sum('sum_abc'))['total'] or 0
    total_balance = total_intake + total_output

    time_range_day = IntakeOutput.objects.filter(
        patient=patientid, time__range=[start_time_day, end_time_day])
    time_range_night = IntakeOutput.objects.filter(patient=patientid, time__range=[
                                                   start_time_night, end_time_night])
    get_lastdate = IntakeOutput.objects.filter(patient=patientid).order_by(
        '-date').exclude(time__isnull=True).values_list('date', flat=True).first()
    intakeoutput_data = IntakeOutput.objects.filter(patient=patientid).filter(
        date=get_lastdate).exclude(time__isnull=True)
...
class IntakeOutputCreateView(BSModalCreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            data['formset'] = IntakeOutput_ModelFormSet(
...
class IntakeOutputUpdateView(BSModalUpdateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields['intake_parenteral_type'].label = _(
            "Intake Parenteral Type")
