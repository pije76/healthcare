...
#	ec_phone = forms.CharField(required=False, label="", widget=forms.TextInput(attrs={'class': "form-control"}))
	...
	own_medication_frequency = forms.ChoiceField(required=False, label="", initial=False, widget=forms.Select(attrs={'class': "form-control"}), choices=Admission.MEDICATION_ADMINISTRATION_FREQUENCY_CHOICES)
...
class AdmissionFormSetForm(BSModalForm):
	#	class Meta:
	#		model = Admission
