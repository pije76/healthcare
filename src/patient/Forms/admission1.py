...
#	AdmissionFormSet = inlineformset_factory(Admission, form=Admission_ModelForm, fields=['ec_name', 'ec_ic_number', 'ec_relationship', 'ec_phone', 'ec_address'], extra=0, can_delete=True)


class Patient_ModelForm(BSModalModelForm):
	class Meta:
		model = UserProfile
		fields = '__all__'
		widgets = {
...
#		self.fields['old_transaction'].queryset = queryset
	...
	patient = forms.CharField(required=False, label="", widget=forms.TextInput(attrs={'class': "form-control"}))
	username = forms.CharField(required=False, label="", widget=forms.TextInput(attrs={'class': "form-control"}))
	password = forms.CharField(required=False, label="", widget=forms.TextInput(attrs={'class': "form-control"}))
	first_name = forms.CharField(required=False, label="", widget=forms.TextInput(attrs={'class': "form-control"}))
...
class Patient_Form(BSModalForm):
	def __init__(self, *args, **kwargs):
#		queryset = kwargs.pop('choices')
		super().__init__(*args, **kwargs)
#		self.fields['old_transaction'].queryset = queryset
...
class EmergencyContact_Form(BSModalForm):
