...
#		super().__init__()
    ...
    id = forms.CharField(required=False, label="", widget=forms.HiddenInput(
...
#	date_time = forms.SplitDateTimeField(required=False, label="", initial=timezone.now, input_date_formats=settings.DATE_INPUT_FORMATS, input_time_formats=settings.TIME_INPUT_FORMATS, widget=forms.SplitDateTimeWidget(date_format="%d-%m-%Y", time_format="%H:%M", date_attrs={'class': 'form-control datepickerinput'}, time_attrs={'class': 'form-control timepickerinput'}, attrs={'class': "form-control"}))
    ...
    discharge_status = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_STATUS)
    ...
    nasogastric_tube = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
#	nasogastric_tube = forms.ChoiceField(required=False, label="", widget=forms.RadioSelect(attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
    urinary_catheter_date = forms.DateField(required=False, label="", initial=get_today, input_formats=settings.DATE_INPUT_FORMATS,
                                            widget=DatePickerInput(format="%d-%m-%Y", attrs={'class': "form-control"}))
    urinary_catheter = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
    surgical_dressing_intact = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
    spectacle_walking_aid_denture = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
    appointment_card_returned = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
    own_medication_return = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_CHECKLIST)
    medication_reconcilation = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control", 'style': "display: inline-flex;"}), choices=DISCHARGE_CHECKLIST)
    medication_reconcilation_patient = forms.CharField(
        required=False, label="", widget=forms.TextInput(attrs={'class': "form-control"}))
    given_by = forms.CharField(required=False, label="", widget=forms.TextInput(
...
class DischargeCheckList_Form_Set(forms.Form):
    ...
    patient = forms.CharField(required=False, label="", widget=forms.HiddenInput(
        attrs={'class': "form-control"}))
    date_time = forms.SplitDateTimeField(required=False, label="", initial=timezone.now, input_date_formats=settings.DATE_INPUT_FORMATS, input_time_formats=settings.TIME_INPUT_FORMATS, widget=forms.SplitDateTimeWidget(
        date_format="%d-%m-%Y", time_format="%H:%M", date_attrs={'class': 'form-control datepickerinput'}, time_attrs={'class': 'form-control timepickerinput'}, attrs={'class': "form-control"}))
    discharge_status = forms.MultipleChoiceField(required=False, label="", widget=forms.CheckboxSelectMultiple(
        attrs={'class': "form-control"}), choices=DISCHARGE_STATUS)
    nasogastric_tube_date = forms.DateField(required=False, label="", initial=get_today, input_formats=settings.DATE_INPUT_FORMATS,
                                            widget=DatePickerInput(format="%d-%m-%Y", attrs={'class': "form-control"}))
