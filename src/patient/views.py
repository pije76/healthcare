...
def load_ic_number(request):
    ...
    fullname_results = UserProfile.objects.filter(
        full_name=fullname_data).order_by('full_name')
    familyname_results = Family.objects.filter(
        ec_name=familyname_data).order_by('ec_name')
...
def load_relationship(request):
    ...
    familyname_results = Family.objects.filter(
        ec_name=familyname_data).order_by('ec_name')
    familyrelationship_results = Family.objects.filter(
        ec_name=familyname_data).values_list('ec_relationship', flat=True).first()
...
def load_signature(request):
    ...
    familyname_results = Family.objects.filter(
        ec_name=familyname_data).order_by('ec_name')
    signature_results = Family.objects.filter(
        ec_name=familyname_data).values_list('ec_name', flat=True).first()
...
def load_phone(request):
    ...
    familyname_results = Family.objects.filter(
        ec_name=familyname_data).order_by('ec_name')
    family_phone_results = Family.objects.filter(
        ec_name=familyname_data).values_list('ec_phone', flat=True).first()
...
def patientdata_detail(request, username):
    ...
    patientid = UserProfile.objects.filter(
    ...
    application_home_leave = ApplicationForHomeLeave.objects.filter(
        patient=patientid)
    appointment = Appointment.objects.filter(patient=patientid)
    cannula = Cannula.objects.filter(patient=patientid)
    dischargechecklist = DischargeCheckList.objects.filter(
        patient=patientid).exclude(medication_reconcilation_patient__isnull=True)
    dressing = Dressing.objects.filter(patient=patientid)
    enteralfeedingregime = EnteralFeedingRegime.objects.filter(
        patient=patientid).exclude(time__isnull=True)
    hgt = HGT.objects.filter(patient=patientid)
    intakeoutput = IntakeOutput.objects.filter(
        patient=patientid).exclude(time__isnull=True)
    investigation_report = InvestigationReport.objects.filter(
        patient=patientid)
...
#	medicationadministrationrecord = MedicationAdministrationRecord.objects.filter(patient=patientid).exclude(medication_template__medication_time__isnull=True)
    medicationadministrationrecord = MedicationAdministrationRecord.objects.filter(
        patient=patientid)
    medicationtemplate = MedicationAdministrationRecordTemplate.objects.filter(
        patient=patientid).exclude(medication_time__isnull=True)
    ...
    miscellaneouschargesslip = MiscellaneousChargesSlip.objects.filter(
        patient=patientid)
    nasogastric = Nasogastric.objects.filter(patient=patientid)
    nursing = Nursing.objects.filter(patient=patientid)
    physioprogressnotesheet = PhysioProgressNoteSheet.objects.filter(
        patient=patientid)
    physiotherapygeneralassessment = PhysiotherapyGeneralAssessment.objects.filter(
        patient=patientid)
