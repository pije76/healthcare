...
@login_required
def save_medication_data_form(request, form, template_name):
    data = dict()
...
def medication_data(request, username):
    ...
    return render(request, 'patient_data/medication_data/medication_data.html', context)
...
def medication_data_delete(request, id):
    ...
    if request.method == 'POST':
        ...
        data['html_form'] = render_to_string('patient_data/medication_data/partial_delete.html', context, request=request)
        return JsonResponse(data)
