...
class MyLoginForm(LoginForm):
    ...
    password = forms.CharField(max_length=100, required=True, label=_(
        'Password'), widget=forms.PasswordInput(attrs={'class': "form-control", 'placeholder': _("Password")}))


class MySignUpForm(SignupForm):

    username = forms.CharField(max_length=100, required=True, label=_(
        'Username:'), widget=forms.TextInput(attrs={'class': "form-control"}))
    full_name = forms.CharField(max_length=100, required=False, label=_(
        'Full Name:'), widget=forms.TextInput(attrs={'class': "form-control"}))
...
class ChangeUserProfile(forms.ModelForm):
    ...
    full_name = forms.CharField(max_length=100, required=True, label='',
                                widget=forms.TextInput(attrs={'class': "form-control"}))
    email = forms.EmailField(required=False, label='', widget=forms.TextInput(
        attrs={'class': "form-control"}))
    ic_number = forms.CharField(max_length=14, required=False, label="", initial='yymmdd-xx-zzzz',
                                validators=[ic_number_validator], widget=forms.TextInput(attrs={'class': "form-control"}))
...
class PatientProfile_ModelForm(forms.ModelForm):
    ...
    full_name = forms.CharField(required=False, label="", widget=forms.TextInput(
        attrs={'class': "form-control"}))
    username = forms.CharField(required=False, label="", widget=forms.HiddenInput(
        attrs={'class': "form-control"}))
    is_patient = forms.BooleanField(
        required=False, label='', widget=forms.HiddenInput())
    is_active = forms.BooleanField(
        required=False, label='', widget=forms.HiddenInput())
    email = forms.EmailField(required=False, label="", widget=forms.HiddenInput(
        attrs={'class': "form-control"}))
    password = forms.CharField(required=False, label="", widget=forms.HiddenInput(
        attrs={'class': "form-control"}))
    date_joined = forms.DateTimeField(required=False, label="", input_formats=settings.DATETIME_INPUT_FORMATS,
    ...
    ic_number = forms.CharField(max_length=14, required=False, label=_('IC No:'), validators=[
                                ic_number_validator], widget=forms.TextInput(attrs={'class': "form-control", 'placeholder': _("yymmdd-xx-zzzz")}))
    ...
    age = forms.CharField(required=False, label="", widget=forms.TextInput(attrs={
                          'class': "form-control", 'readonly': 'readonly', 'placeholder': _("*auto fill-in")}))
