...
$('#demo-btn-addrow').on('click', function()
{
	t.row.add([
		$('#firstName').val(),
		$('#lastName').val(),
		'<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline edit-row-btn" data-toggle="modal" data-target="#rowEditModal" data-whatever="@getbootstrap" data-original-title="Edit" style="background-color:#272b34;"><i class="ti-pencil text-info m-r-10"></i></button><button  type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" style="background-color:#272b34;" ><i class="ti-trash text-danger m-r-10"></i></button>'
	]).draw(false);
});

$('#t_add_row tbody').on('click', 'tr', function()
{
	...
	{ 
		...
		jQuery.each(columns, function(i, item) {
			...
			arrbuf[i]=item.innerHTML;
		});
		$("#firstName_edit").val(arrbuf[1]);
		$("#lastName_edit").val(arrbuf[2]);
	}
});
...
$('#demo-btn-updaterow').on('click', function() {
	t.row('.selected').remove().draw(false);
	t.row.add([
		$('#firstName_edit').val(),
		$('#lastName_edit').val(),
		'<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline edit-row-btn" data-toggle="modal" data-target="#rowEditModal-{{ forloop.counter }}" data-whatever="@getbootstrap" data-original-title="Edit" style="background-color:#272b34;"><i class="ti-pencil text-info m-r-10"></i></button><button  type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" style="background-color:#272b34;" ><i class="ti-trash text-danger m-r-10"></i></button>'
	]).draw(false);
});
